...
void menu(Trainer &player){//main hub of the game, where trainer either encounters pokemon, forages for new items, or fast travels to a new location.
    ...
    std::cout << "1.)Forage" << std::endl << std::endl;
    std::cout << "2.)Explore" << std::endl << std::endl;
    std::cout << "3.)Fast Travel to another region";
        if(player.trainerLvl < 5)
            std::cout << "(Unlocked at Lvl 5)" << std::endl << std::endl;
        else{
...
int main() {
    ...
    switch(userChoice){//used to identify which starter the user will have acess to depending on their region of choice
        ...
        case 3: player.currRegion = "Hoenn";
